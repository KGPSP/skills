...
**Reference implementation:** kopiuj `dev/audited-feature-workflow/` jako wzorzec dojrzałego skilla (frontmatter, fazy z exit criteria, `references/` z `source:` per sekcja DOC, `scripts/` POSIX). `dev/feature-spec-planner/` to przykład skilla pochodnego (odcięcie faz wykonawczych od v3).

## Czym jest to repo

Katalog **Codex Skills** dla Komendy Głównej PSP. Repo **nie zawiera aplikacji** — każdy skill to samodzielny pakiet dokumentacji proceduralnej (Markdown + skrypty POSIX), który Codex ładuje na podstawie frontmatter `name`/`description`. „Build" tutaj = napisanie/edycja skilla zgodnego z pryncypiami DOC, nie kompilacja kodu.
...
Repo jest **instalowalnym marketplace** — `.Codex-plugin/marketplace.json` (nazwa `kgpsp-skills`) grupuje skille w 3 pluginy wg domen:

- `pzp-tools` → `pzp/.Codex-plugin/plugin.json`
- `legal-tools` → `legal/.Codex-plugin/plugin.json`
- `dev-tools` → `dev/.Codex-plugin/plugin.json`

Każdy `plugin.json` ma pole **`skills:`** wskazujące katalogi skilli **wprost** (np. `"./sejm-eli-api"`) — bez przenoszenia do `skills/`. Pole jest additywne do domyślnego `skills/`; nazwa wywołania pochodzi z `name:` we frontmatterze SKILL.md (wygrywa z nazwą katalogu — choć obecnie wszystkie skille mają nazwę katalogu zsynchronizowaną z `name:`). Cały drzewostan domeny kopiowany jest do cache, więc `references/`/`scripts/`/`templates/` działają; **ścieżki `../` poza katalog pluginu nie działają** po instalacji (dlatego skille nie mogą zależeć od `DOC/` w runtime — `DOC/` to źródło autorskie, nie runtime).
...
- **`allowed-tools`:** wymień tylko narzędzia naprawdę potrzebne. Skill wyłącznie analizujący/planujący NIE dostaje `Edit`/`Write` na kodzie (przykład: `feature-spec-planner` ma `Write` tylko do artefaktów planistycznych).
...
# Bramka kompletności planu (feature-spec-planner) — sprawdź na fixtures
sh dev/feature-spec-planner/scripts/check-plan-complete.sh --plan dev/feature-spec-planner/tests/fixtures/complete-plan.md    # exit 0
sh dev/feature-spec-planner/scripts/check-plan-complete.sh --plan dev/feature-spec-planner/tests/fixtures/incomplete-plan.md  # exit 1
...
Codex plugin validate .                                  # bramka manifestu — musi: ✔ Validation passed
Codex plugin marketplace add /Users/sq13pl/Documents/GitHub/skills   # test lokalny ze ścieżki
Codex plugin install <plugin>@kgpsp-skills && Codex plugin details <plugin>   # potwierdza Skills (N)
Codex plugin uninstall <plugin> && Codex plugin marketplace remove kgpsp-skills   # SPRZĄTAJ po teście
```
...
- Commit messages kończ stopką `Co-Authored-By: Codex Opus 4.7 (1M context) <noreply@anthropic.com>`.
- `DOC/` i `.Codex/*.state`/`*.local.*` są w `.gitignore` — nie próbuj ich commitować.

## Pełniejszy kontekst

- Indeks i porównanie skilli: [`README.md`](README.md), decision tree planerów: [`dev/README.md`](dev/README.md).
- Pozycjonowanie wariantów planowania/wykonania feature'a: `replit-style-workflow` (wygodny, pełen workflow plan+impl+test+review+ADR — historycznie feature-planner-v2) · `audited-feature-workflow` (senior-grade, pełny SDLC z deterministyczną uprzężą) · `feature-spec-planner` (planning-only, kończy na planie+ADR, handoff do wykonawcy).
