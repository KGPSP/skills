...
| [`audited-feature-workflow`](dev/audited-feature-workflow/) | **senior-grade** (Claude Code, Opus 4.8) · `v3.10.0` | `replit-style-workflow` + deterministyczna uprząż inżynieryjna: 15-wpisowa Anti-Rationalization Table, twardy DoD z surowymi artefaktami, PR Sizing 100/300/1000, Hyrum's Law, Chesterton's Fence, Beyoncé Rule 1:1 AC↔Test, DAMP over DRY, Five-Axis Review, Plan-Validate-Execute, Thin Vertical Slices, Prove-It Pattern, **xhigh + Dynamic Workflows orchestration** jako standard Phase 1/6/8 (bramka deklaracji `orchestration:` + szablony `.js`). 17 faz + 6 bramek HITL, 21 deterministycznych bramek-skryptów. **/goal mode** — autonomiczna pętla weryfikacji AC. Dla zadań **wysokiego rygoru** — fragile ops, audytowalna delegacja, compliance. (Historycznie: `feature-planner-v3`.) |
...
| [`qa-architect`](dev/qa-architect/) | **v1.0.1** | QA blueprint **setup-time** (nie wykonuje testów): generator strategii QA + konfiguracji dla multi-stack (Next.js/React, Node, Python, Go + PostgreSQL). Orkiestruje Managera + 5 sub-agentów (tooling-decisor, config-builder, test-author, ci-author, reviewer) wg paradygmatu QA-swarm. 8 faz + 2 bramki approval. Produkuje `qa-blueprint/` (24 pliki: qa-strategy + configs + samples + CI + CLAUDE.md.patch + AGENTS.md + verify-tests skill + checklists + pilot 4-tyg). Wymusza realny Postgres via Testcontainers + Playwright. Aktywuj `/qa-architect`. |
...
| `dev-tools` | 7 | agent-teams-builder, replit-style-workflow, audited-feature-workflow, feature-spec-planner, playwright-test-suite, qa-architect, swarm-orchestrator |
