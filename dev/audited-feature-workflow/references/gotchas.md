...
> Ten plik **rośnie z czasem**. Każde uruchomienie v3 w Phase 1 dokleja anomalia odkryte w analizie kodu. Phase 9 dedupuje. Po >100 wpisach — split per moduł (`gotchas-api.md`, `gotchas-ui.md`, `gotchas-db.md`).
...
> [!important] Poniższe 3 wpisy są szablonowe — Phase 1 v3 zastępuje je realnymi anomalia z projektu.
...
|---|---|
| **Phase 1** | **Add pass** — po analizie kodu/struktury projektu wykrywa nowe anomalia i dopisuje wpisy (sekcja 2 template). Lokalizacje weryfikowane (`test -e`). |
| **Phase 4** | Plan referencyjnie wskazuje gotchas relewantne dla feature'a (np. „dotyczy soft-delete — patrz gotchas.md#soft-delete-via-deleted_at"). |
| **Phase 6** | Anti-rationalization quick-check przed commit — czy implementacja respektuje wszystkie gotchas wymienione w planie? Jeśli nie → STOP. |
| **Phase 8** | Five-axis review (oś Architecture) sprawdza zgodność z gotchas — naruszenie = Critical finding. |
| **Phase 9** | **Dedup pass** — sekcja 4 reguły utrzymania. Sortowanie, merge duplikatów, flag stale, ewentualny split per moduł przy >100 wpisach. |
