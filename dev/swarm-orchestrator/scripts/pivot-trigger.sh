...
if [[ "${PIVOT_REQUIRES_HUMAN:-1}" == "1" ]]; then
  ...
  if [[ "${ANS,,}" != "y" ]]; then
    ...
    BASE_DIR="$BASE_DIR" bash "$SCRIPTS_DIR/append-breadcrumb.sh" "human" "pivot_aborted" \
...
# Breadcrumb
BASE_DIR="$BASE_DIR" bash "$SCRIPTS_DIR/append-breadcrumb.sh" "pivot-trigger" "pivot_executed" \
