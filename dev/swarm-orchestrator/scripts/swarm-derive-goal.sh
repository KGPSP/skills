...

{
  ...
  echo "### BUILD — clean"
  echo "- **Komenda**: \`sh dev/audited-feature-workflow/scripts/verify-build-clean.sh\`"
  echo "- **Próg**: exit 0, zero warnings."
  echo ""
  echo "### AC-COVERAGE — 1:1"
  echo "- **Komenda**: \`sh dev/audited-feature-workflow/scripts/check-ac-coverage.sh --plan \"${PLAN}\"\`"
  echo "- **Próg**: 100%."
  echo ""
  echo "## Ograniczenia"
  echo ""
